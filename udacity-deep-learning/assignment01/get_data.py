...
class GetData:
  ...
  limited to 'A' through 'J' (10 classes). The training set has about 500k and the
  testset 19000 labeled examples. Given these sizes, it should be possible to
  train models quickly on any machine.
  '''
  def __init__(self):
    pass
  ...
  def maybe_download(self, filename, expected_bytes, force=False):
    ...
    if force or not os.path.exists(dest_filename):
      ...
      filename, _ = urlretrieve(url + filename, dest_filename, reporthook=self.download_progress_hook)
      print('\nDownload Complete!')
  ...
  '''
  Extract the dataset from the compressed .tar.gz file. This should give you a set
  of directories, labeled A through J.
  '''
  ...
  '''
  def load_letter(self, folder, image_size, pixel_depth, min_num_images):
  ...
  '''
  def maybe_pickle(self, data_folders, image_size, pixel_depth, min_num_images_per_class, force=False):
    dataset_names = []
    for folder in data_folders:
      ...
      if os.path.exists(set_filename) and not force:
        ...
        dataset = self.load_letter(folder, image_size, pixel_depth, min_num_images_per_class)
        try:
  ...
  you might not be able to fit it all in memory, and you can tune train_size
  as needed. The labels will be stored into a separate array of integers 0
  through 9.
  Also create a validation dataset for hyperparameter tuning.
  '''
  def merge_datasets(self, pickle_files, train_size, image_size, valid_size=0):
  ...
  '''
  def save_data(self, train_dataset, train_labels, valid_dataset, valid_labels, test_dataset, test_labels):
